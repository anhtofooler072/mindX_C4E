...
function sum(cartProduct) {
    for (let i=0;i<cartProduct.length;i++) {
        cartProduct[i].pItem.discount==1 ? total+=(cartProduct[i].pItem.discountPrice*cartProduct[i].sl) : total+=(cartProduct[i].pItem.price*cartProduct[i].sl)
        console.log(total);
    }
}
sum(cartProduct)
function render(cartProduct) {
    ...
    for (let i=0;i<cartProduct.length;i++) {
        ...
        <tr class="cart">
            ...
            <td class="product-subtotal" id="productSub${i}">$${(cartProduct[i].pItem.price*cartProduct[i].sl)}</td>
...
function plus(i) {
    ...
    cartProduct[i].pItem.discount==1 ? selectedPrice=cartProduct[i].pItem.discountPrice : selectedPrice=cartProduct[i].pItem.price
    document.querySelector(`#${dynamicIdQtt}`).value=cartProduct[i].sl
    document.querySelector(`#${dynamicIdSub}`).innerHTML=`$${cartProduct[i].sl*selectedPrice}`
    total=0
    sum(cartProduct)
    console.log(cartProduct);
